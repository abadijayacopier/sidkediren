...
export default function PrintApbdesPage() {
  ...
  return (
    <div className="bg-white min-h-screen p-8 md:p-16 font-serif text-black leading-tight text-[11px] print:p-0">
      ...
      <div className="no-print mb-10 bg-slate-100 p-4 rounded-xl flex justify-between items-center max-w-4xl mx-auto border border-slate-200">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-emerald-600 text-white rounded-xl flex items-center justify-center font-black">SID</div>
          <div>
            <p className="text-sm font-black text-slate-800">Pratinjau Cetak APBDes</p>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Siskeudes Official Format</p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={() => window.close()} className="px-6 py-2 bg-white text-slate-600 border border-slate-200 rounded-lg font-bold hover:bg-slate-50 transition-all">Tutup</button>
          <button onClick={() => window.print()} className="px-6 py-2 bg-slate-800 text-white rounded-lg font-bold hover:bg-slate-900 transition-all shadow-lg shadow-slate-200">Cetak Sekarang</button>
        </div>
      </div>
      ...
      <div className="max-w-[210mm] mx-auto bg-white">
        ...
        <div className="mt-8 flex justify-between items-start text-[9px]">
          ...
          <div className="text-center w-[200px]">
            <p className="mb-16">Kepala Desa {profil?.namaDesa || 'Kediren'}</p>
            <p className="font-bold underline uppercase">{profil?.namaKepalaDesa || '__________________________'}</p>
          </div>
        </div>
      </div>
