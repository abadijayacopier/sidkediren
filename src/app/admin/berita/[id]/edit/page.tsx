...
import { updateBerita, getBeritaById } from '@/app/actions/berita';
...
export default function EditBeritaPage() {
  ...
  useEffect(() => {
    getBeritaById(id).then(res => {
      setBerita(res);
      if (res?.gambar) setImagePreview(res.gambar);
      setLoading(false);
    });
  }, [id]);
  ...
  if (!berita) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/admin/berita" className="text-emerald-600 font-bold mt-4 block">Kembali ke list</Link>
      </div>
    );
  }

  return (
    <form action={updateBerita} className="max-w-4xl mx-auto space-y-8 pb-20">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Edit Berita</h1>
            <p className="text-slate-500 text-sm font-medium">Perbarui informasi yang telah dipublikasikan.</p>
          </div>
        </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
            <div>
              ...
              <select 
                ...
                className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none font-bold text-slate-700 text-sm appearance-none cursor-pointer"
              >
            ...
            <div className="flex items-center justify-between p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
               ...
               <div className="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" name="isPublished" defaultChecked={berita.isPublished} className="sr-only peer" />
