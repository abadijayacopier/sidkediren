...
import { Landmark, Plus, MapPin, Search, Edit3, Trash2, X, Save, Calendar, Users, DollarSign, ArrowLeft, Layers, CheckCircle } from 'lucide-react';
import { getKegiatanList, saveKegiatan, getKaderPkkList, deleteKegiatan } from '@/app/actions/pkk';
import { motion, AnimatePresence } from 'framer-motion';
import Swal from 'sweetalert2';

export default function Pokja1Page() {
  ...
  // Form State (Kategori dikunci ke "Pokja I")
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Filter sejak awal untuk Pokja I
      const filtered = ((kegiatanRes || []) as any[]).filter(k => k.kategori === 'Pokja I');
      setDataKegiatan(filtered);
      setKaderList((kaderRes as any[]) || []);
    } catch (error) {
      console.error('Failed to load Pokja I data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('kategori', 'Pokja I'); // LOCK KATEGORI
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/admin/pkk" className="p-3 bg-white border border-slate-100 rounded-2xl text-slate-400 hover:text-sky-600 transition-all shadow-sm">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-3">
              ...
              Buku Kegiatan Pokja I
            </h1>
            <p className="text-slate-500 text-xs mt-1 font-semibold">
              Pembinaan Karakter Keluarga, Pola Asuh Anak & Remaja (PAAR), Gotong Royong, dan Nilai Sosial.
            </p>
          </div>
        </div>
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs px-5 py-3.5 rounded-2xl transition-all shadow-lg shadow-sky-100 uppercase tracking-widest"
        >
          <Plus size={16} /> Tambah Log Pokja I
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-4 flex items-center justify-between gap-4">
        <div className="relative max-w-md w-full">
          ...
          <input 
            ...
            className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 focus:bg-white transition-all text-xs font-semibold outline-none text-slate-700"
          />
        </div>
        <div className="px-4 py-2 bg-sky-50 text-sky-700 rounded-xl text-[10px] font-black uppercase tracking-widest border border-sky-100">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-3">
            ...
            <span className="text-xs font-bold">Memuat database kegiatan Pokja I...</span>
          </div>
        ) : filteredKegiatan.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-3">
            <Layers size={40} className="text-slate-300" />
            <span className="text-xs font-bold">Belum ada log kegiatan untuk Pokja I.</span>
          </div>
        ) : (
              ...
              <div key={item.id} className="p-8 hover:bg-slate-50/50 transition-all flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                <div className="space-y-3 max-w-3xl">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="px-2.5 py-0.5 border bg-sky-50 text-sky-600 border-sky-100 rounded-full text-[9px] font-black uppercase tracking-wider">
                      {item.kategori}
                    </span>
                  ...
                  <div className="flex flex-wrap gap-x-6 gap-y-2 pt-2 text-[11px] text-slate-500 font-bold">
                    ...
                    {item.kader && (
                      <span className="flex items-center gap-1.5"><CheckCircle size={13} className="text-sky-500" /> PJ: {item.kader.nama} ({item.kader.jabatan})</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 self-end lg:self-start">
                  <button 
                    ...
                    className="p-2.5 bg-slate-50 hover:bg-sky-50 hover:text-sky-600 text-slate-500 rounded-xl transition-all"
                  >
                    <Edit3 size={15} />
                  </button>
                  <button 
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">
                      {formId ? 'Edit Laporan Pokja I' : 'Tambah Laporan Pokja I'}
                    </h3>
                    <p className="text-slate-400 text-[10px] uppercase font-bold tracking-widest mt-0.5">
                      Program Sosial & Nilai Karakter
                    </p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Nama Laporan Kegiatan Pokja I *</label>
                    <input 
                      type="text" 
                      value={formNama} 
                      onChange={(e) => setFormNama(e.target.value)} 
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-200 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 rounded-xl outline-none font-semibold text-slate-700 text-sm transition-all"
                      placeholder="Contoh: Pengajian Rutin Dusun Selungguh" 
                      required 
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      required 
                    />
                    <datalist id="program-p1-list">
                      <option value="Penyuluhan Pola Asuh Anak (PAAR)" />
                      <option value="Gotong Royong Kebersihan Lingkungan" />
                      <option value="Pengajian Rutin Dusun" />
                      <option value="Pembinaan Kesadaran Bela Negara (PKBN)" />
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-200 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 rounded-xl outline-none font-semibold text-slate-700 text-sm transition-all"
                      placeholder="Contoh: Masjid Baiturrahman Dusun Selungguh" 
                      required 
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none font-semibold text-slate-700 text-sm"
                    >
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea 
                      ...
                      rows={4}
                      className="w-full px-5 py-3 bg-slate-50 border border-slate-200 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 rounded-xl outline-none font-semibold text-slate-700 text-sm transition-all"
                      placeholder="Uraikan detail kegiatan Pokja I, tujuan, kendala, dan hasil yang diperoleh..."
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                  ...
                  <button 
                    type="submit" 
                    className="flex items-center gap-2 px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs rounded-xl transition-all shadow-lg shadow-sky-100"
                  >
