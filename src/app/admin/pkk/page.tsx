...
import { HeartPulse, Users, Activity, FileText, Plus, Bell, Calendar, MapPin, Search, ChevronRight, Scale, Baby, Stethoscope, Save, Trash2, X } from 'lucide-react';
import { getJadwalPosyandu, getBalitaKmsList, getKaderPkkList, seedPkkData, saveBalita, deleteBalita, getPosyanduList } from '@/app/actions/pkk';
import { motion, AnimatePresence } from 'framer-motion';
import Swal from 'sweetalert2';

export default function PkkPosyanduPage() {
  const [activeTab, setActiveTab] = useState<'jadwal' | 'balita' | 'kader'>('jadwal');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [jadwalRes, balitaRes, kaderRes, posyanduRes] = await Promise.all([
        getJadwalPosyandu(),
        getBalitaKmsList(),
        getKaderPkkList(),
        getPosyanduList()
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Sistem Informasi e-KMS, data kesehatan balita, ibu hamil, dan kegiatan PKK.</p>
        </div>
      ...
      {/* Stats - Diperbarui agar lebih interaktif */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <StatCard icon={<Scale size={24} />} label="Gizi Kurang/Stunting" value={dataBalita.filter(b => b.statusGizi !== 'Normal').length || 0} suffix="Anak" color="bg-amber-50" textColor="text-amber-600" />
        <StatCard icon={<Stethoscope size={24} />} label="Kegiatan Posyandu" value={jadwalPosyandu.length || 0} suffix="Bulan Ini" color="bg-blue-50" textColor="text-blue-600" />
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
         ...
         <div className="flex items-center border-b border-slate-100 bg-slate-50/50 p-2">
            ...
            <TabButton 
              ...
              icon={<Baby size={16} />} 
              label="Data e-KMS Balita" 
            />
            <TabButton 
              active={activeTab === 'kader'} 
              onClick={() => setActiveTab('kader')} 
              icon={<Users size={16} />} 
              label="Daftar Kader PKK" 
            />
         </div>
         ...
         <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white">
           <div className="relative max-w-md w-full">
             ...
             <input 
               type="text" 
               placeholder="Cari nama, dusun, atau status..." 
           ...
           <div className="flex items-center gap-3">
             <select className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-600 focus:outline-none focus:ring-2 focus:ring-rose-500">
         ...
         <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-xs text-slate-500 font-medium">
            <p>Menampilkan 1 hingga 10 dari total data</p>
            <div className="flex items-center gap-2">
              <button className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg hover:bg-slate-100 disabled:opacity-50">Sebelumnya</button>
              <button className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg hover:bg-slate-100">Selanjutnya</button>
            </div>
         </div>
      </div>
